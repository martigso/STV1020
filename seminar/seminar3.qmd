...
# Seminar 3 -- Omkoding, subsetting og visualisering
...
Når vi omkoder variabler i et datasett, bør vi opprette en ny variabel. Dersom vi ikke gjør dette, erstatter vi informasjonen i den opprinnelige variabelen. Vi trenger informasjonen i den opprinnelige variabelen for å sjekke at omkodingen har fungert som vi ønsker. Vi kan også gjøre feil som vi ikke kan rette opp uten den opprinnelige variabelen. Først bruker jeg funksjonene `names()`, `head()` og `summary()` for å få litt oversikt over data. Du kan også trykke på datasettet i environment:
...
table((gapminder$year_1952 + 1952) == gapminder$year, useNA = "always")
```
...
table(gapminder$lifeExp_2, gapminder$lifeExp > mean(gapminder$lifeExp, na.rm = TRUE), useNA = "always")
