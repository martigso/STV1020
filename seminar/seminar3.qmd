...
Mange av ekstrapakkene vi bruker i R har med seg innebygde datasett -- ofte fordi de kan brukes til å illustrere hva funksjonene i pakken gjør, men noen pakker er også "datapakker". I dag skal vi bruke en slik pakke: `gapminder`. Du kan lese mer om bakgrunnen til Gapminder [her](https://www.gapminder.org/).

For å bruke data i pakken, må vi først installere pakken og åpne den. Husk fra [seminar 1](seminar1.qmd) at man bare trenger å bruke `install.packages()` en gang, mens man må bruke `library()` hver gang man starter en ny `rsession` (hver gang du åpner RStudio etter du har lukka det).

```{r installere_gapminder, eval = FALSE}
install.packages("gapminder")
```

```{r laste_inn_gapminder}
library(gapminder)
```
...
For å fjerne alt så kan du også trykke på ikonet med en feiekost på i environment. Merk at dersom du ikke har lagret objektene på maskinen din så må du kjøre koden på nytt for å få tilbake de objektene du fjerner. Her fjerner vi dataobjektet vårt før vi laster det inn på nytt ved hjelp av `load()`:
...
```{r mutate_omkoding, message=FALSE}
library(tidyverse)
...
Du kan også bruke funksjonen `filter()` til å velge ut spesielle observasjoner som tilfredsstiller de kravene du setter. Under har vi brukt funksjonen til å bare ta med observasjonene med verdien 1954 på variabelen `year` og som har en verdi høyere end gjennomsnittet på variablene `pop`:
...
Vi kan legge til en linje som viser trenden^[`geom_smooth()` bruker (vanligvis) lokale regresjoner for å beregne linja.] i observasjonene ved å bruke geom_smooth: 

```{r geom_smooth, eval = TRUE}
ggplot(gapminder, aes(x = lifeExp, y = gdpPercap)) +
  geom_point() + 
  geom_smooth()
...
```{r farge_points_smooth, eval = TRUE}
...
```{r tweeks_smooth_figur, eval = TRUE}
ggplot(gapminder, aes(x = lifeExp, y = gdpPercap, col = continent)) +
  geom_point() + 
...
```{r endre_titler_figur, eval = TRUE}
ggplot(gapminder, 
...
- [ggplot2 dokumentasjon](https://ggplot2.tidyverse.org/)
- [Healy sin nettside](https://socviz.co/makeplot.html#makeplot)
- [ggplot2 tutorial med gapminder data](http://euclid.psych.yorku.ca/www/psy6135/tutorials/gapminder.html)
- [Tidy Tuesday](https://www.tidytuesday.com/)
- [#tidytuesday på twitter](https://twitter.com/hashtag/tidytuesday)
 
```{r generere_script, eval=FALSE, echo=FALSE}
knitr::purl("./seminar/seminar3.qmd", 
            output = "./src/seminar3.R", 
            documentation = 1)
```